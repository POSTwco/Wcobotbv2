...
const LETTER_STAGES: LetterStage[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "1. Make your edit (new cue, better description, custom bucket URL, new exercise, dose tweak, tempoHint or scaling names).",
      "2. Hit Save to Live Engine. Wait for the success banner and library reload. Watch console for any 'STALE BUNDLE' or roundtrip warnings.",
      ...
      "6. Verify: cues match exactly what you wrote, description appears as Educational note, image shows if you set a custom URL (check both genders via rail toggle), name/dose/tempo correct, scaling ladder names if customized.",
      ...
      "KNOWN SAVE FAILURE (as of latest audit): 'saveCaliOverride is not a function' = stale Vercel frontend bundle. Git push of functions alone is not enough. Must trigger full Vite rebuild on Vercel.",
      "REDEPLOY STEPS: In Wcobotbv2 run `npm run build`, git add the admin + api files, commit, `git push origin main`. Wait for Vercel build + Supabase fn deploy. Hard refresh admin page + inspect Network for fresh JS containing the save methods.",
