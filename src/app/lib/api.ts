...
// Ensure Calisthenics admin methods are always attached to api.admin (fixes "is not a function" in some bundles/HMR)
const caliAdminMethods = {
  getCaliStats: (adminWallet: string, sessionToken: string) =>
    request<{
      totalProfiles: number;
      totalWorkouts: number;
...
Object.assign((api as any).admin, caliAdminMethods);

// Also ensure on the main api if needed for some imports
if (!(api as any).getCaliLibrary) {
  (api as any).getCaliLibrary = caliAdminMethods.getCaliLibrary;
  (api as any).getCaliStats = caliAdminMethods.getCaliStats;
}