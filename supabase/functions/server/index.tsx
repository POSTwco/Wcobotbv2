 ...
 *   POST   /sponsors/:id/impression  Track impression
 *   POST   /sponsors/:id/click       Track click
...
app.post(`${PREFIX}/admin/sponsors`, requireAdminSession, async (c) => {
  try {
    ...
    const sponsor = {
      ...
      tier: ["title", "premium", "standard"].includes(body.tier) ? body.tier : (existing?.tier || "standard"),
      tiers: Array.isArray(body.tiers) ? body.tiers.filter((t: string) => ["title", "premium", "standard"].includes(t)) : (existing?.tiers || [existing?.tier || "standard"]),
...
app.post(`${PREFIX}/sponsors/:id/impression`, async (c) => {
  try {
    ...
    if (!s) return c.json({ success: false }, 404);
